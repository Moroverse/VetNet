...
- [ ] **Task 1.1: StateKit ListModel Implementation**
  - [ ] Create `PatientListModel: ListModel<Patient>` ViewModel
  - [ ] Connect to existing `PatientPaginationRepository.findWithPagination`
  - [ ] Configure ListModel with proper error handling and loading states
  - [ ] Replace PatientListView stub with StateKit integration

- [ ] **Task 1.2: Patient List UI Enhancement**
  - [ ] Design patient card layout with all required information
  - [ ] Implement pull-to-refresh using ListModel capabilities
  - [ ] Add proper loading and empty states
  - [ ] Ensure accessibility compliance with patient cards

- [ ] **Task 1.3: Search Bar UI**
  - [ ] Add SwiftUI SearchableTextField to PatientListView
  - [ ] Style search bar consistently with app design
  - [ ] Implement clear button functionality
  - [ ] Add accessibility identifiers for testing

### Iteration 2: Search Integration
- [ ] **Task 2.1: Search State Management**
  - [ ] Add search text property to PatientListModel
  - [ ] Implement 300ms debounce timer for search input
  - [ ] Switch between findWithPagination and searchByNameWithPagination based on search text
  - [ ] Handle search state transitions smoothly

- [ ] **Task 2.2: Search Results Display**
  - [ ] Connect search bar text to PatientListModel search
  - [ ] Display search result count in UI
  - [ ] Handle empty search results with appropriate messaging
  - [ ] Maintain StateKit ListModel benefits during search

- [ ] **Task 2.3: Search UX Polish**
  - [ ] Implement search text clearing functionality
  - [ ] Add search result highlighting (optional)
  - [ ] Test search performance with existing sample data
  - [ ] Create search interaction tests

### Iteration 3: Search Scoping + Cleanup
- [ ] **Task 3.1: Repository Scope Enhancement**
  - [ ] Add `SearchScope` enum (All, Name, Owner, Species, Breed)
  - [ ] Extend `searchByNameWithPagination` with scope parameter
  - [ ] Implement scope-specific search logic in SwiftData queries
  - [ ] Maintain backward compatibility during transition

- [ ] **Task 3.2: Repository Method Cleanup**
  - [ ] **Remove deprecated methods:**
    - [ ] `searchByName(_ nameQuery: String)`
    - [ ] `findBySpecies(_ species: Species)`
    - [ ] `findByOwnerName(_ ownerName: String)`
  - [ ] Update all references to use scoped search instead
  - [ ] Verify no functionality is lost in migration
  - [ ] Update repository tests to reflect new API

- [ ] **Task 3.3: Scope Selector UI**
  - [ ] Add segmented control for search scope selection
  - [ ] Implement scope state management in PatientListModel
  - [ ] Update search behavior based on selected scope
  - [ ] Style scope selector consistently with app
