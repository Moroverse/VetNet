...
1. **Event Definition Architecture**
   - Create `RouterEvent` protocol extending StateKit's `Event`
   - Define specific router events:
...
- [ ] **Task 1.1: Create RouterEventBroker Implementation**
  - [ ] Create `RouterEventBroker` class implementing StateKit's `EventBroker` protocol
  - [ ] Implement thread-safe subscription management with actor isolation
  - [ ] Add comprehensive logging for event publishing and subscription
  - [ ] Register RouterEventBroker in Container+VetNet with proper scoping

- [ ] **Task 1.2: Router Event Architecture**
  - [ ] Create `RouterEvent` protocol extending StateKit's `Event`
  - [ ] Define specific router events (FormPresentation*, Navigation*, RouterStateChanged)
  - [ ] Add proper event metadata and context information
  - [ ] Create event factory methods for consistent event creation

- [ ] **Task 1.3: EventBroker Integration**
  - [ ] Add EventBroker injection to router via FactoryKit
  - [ ] Replace callback patterns with event publishing in router methods
  - [ ] Ensure proper error handling in event publishing
  - [ ] Add event publishing to all navigation and form presentation methods

- [ ] **Task 1.4: Event Subscription Implementation**
  - [ ] Inject `EventBroker` into ViewModels via FactoryKit
  - [ ] Update `PatientListViewModel` to subscribe to router events
  - [ ] Store `EventSubscription` and handle cleanup in deinit
  - [ ] Add proper memory management with weak self in handlers
...
- **Event**: Protocol for our router events
